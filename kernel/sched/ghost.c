...
/* The legox_txn pointer equals NULL or &enclave->cpu_data[this_cpu].txn */
				...
				bool run_task_reaper, int nr_decrefs)
{
	...
	/* Make sure it enqueued. */
	WARN_ON_ONCE(rq->ghost.ew_head.next == NULL);
}
...
static inline void schedule_next(struct rq *rq, gtid_t gtid, bool resched)
{
	 ...
	 * this function is called via set_curr_task_ghost() but without
...
static void prio_changed_ghost(struct rq *rq, struct task_struct *p, int old)
{
	 ...
	 * __sched_setscheduler -> set_curr_task_ghost guarantees
...
static void set_curr_task_ghost(struct rq *rq)
{
...
int select_task_rq_ghost(struct task_struct *p, int cpu, int sd_flags,
			 int wake_flags)
{
	/*
	 ...
	 *
	 * For now prefer option 1 because it keeps task_cpu(p) coherent
	 * with the agent. We also depend on this behavior to reconcile
	 * the wakeup with 'rq->ghost.switchto_first'.
	 */
	return task_cpu(p);
}
			      ...
			      uint32_t task_barrier, int *state)
{
	...
	/* 'next' must be a runnable (but not running) ghost task. */
						...
						struct rq_flags *rf)
{
	...
	if (next) {
		/*
		 * Restart the pick_next_task() loop so that
		 * pick_next_ghost_agent() gets another chance to pick the
		 * agent. Retry instead of returning 'next' because this is not
		 * the common case and to keep agent picking side-effects in one
		 * place.
		 */
		return RETRY_TASK;
	}
...
pick_latched:
	...
	/* If there is a pending transaction then commit it */
	if (ghost_txn_ready(cpu_of(rq))) {
		WARN_ON_ONCE(rq_adj_nr_running(rq) != 0);
		...
		/*
		 * A higher priority task (including the agent on this cpu)
		 * could have become runnable while runqueue lock was dropped.
		 */
		if (rq_adj_nr_running(rq))
			return RETRY_TASK;

		/*
		 * A runnable agent is reflected in the 'rq_adj_nr_running()'
		 * check above so if we get here then agent must be blocked
		 * (in the kernel or explicitly via ghost_run()).
		 */
		WARN_ON_ONCE(task_on_rq_queued(agent) &&
			     !rq->ghost.blocked_in_run);

		/* txn committed successfully */
		if (rq->ghost.latched_task)
			goto pick_latched;
	}
done:
...
const struct sched_class ghost_sched_class = {
	...
	.set_curr_task		= set_curr_task_ghost,
 ...
 */
const struct sched_class ghost_agent_sched_class = {
	.next			= &dl_sched_class,
...
struct ghost_enclave *ghost_create_enclave(void)
{
	...
	/*
	 * TODO: allocate such that the physical memory for each cpu's page
	 * comes from the NUMA node for that cpu.
	 */
	e->cpu_data = vmalloc_user(GHOST_CPU_DATA_REGION_SIZE);
	if (!e->cpu_data) {
		kfree(e);
		return NULL;
	}
...
void ghost_destroy_enclave(struct ghost_enclave *e)
{
	...
	for_each_cpu(cpu, &e->cpus) {
		...
		spin_unlock_irqrestore(&e->lock, flags);
		send_sig_info(SIGKILL, SEND_SIG_FORCED, agent);
...
/*
 * Holding the struct fd keeps the enclave alive.  If there is an enclave for f,
 * then ghostfs holds a kref for each open file.  Once fdget returns an fd,
 * we've gone from an integer fd to a struct fd, which means we have an open
 * file.  This lasts until fdput(f).
 */
static struct ghost_enclave *fd_to_enclave(struct fd f)
{
	if (f.file)
		return ghostfs_ctl_to_enclave(f.file);
	return NULL;
}
			     ...
			     bool forked)
{
	...
	/* Associate task with the default queue */
	q = e->def_q;
	if (!q) {
		error = -ENXIO;
		goto done;
			     ...
			     bool forked)
{
	...
	spin_lock_irqsave(&e->lock, irq_fl);
	error = __ghost_prep_task(e, p, forked);
	spin_unlock_irqrestore(&e->lock, irq_fl);
			    ...
			    struct rq *rq)
{
	...
	ret = __ghost_prep_task(e, p, false);
	if (ret)
		goto out;
...
static int ghost_schedattr_to_fd(const struct sched_attr *attr)
{
	/* Reusing SCHED_DEADLINE's attr field */
		       ...
		       int *reset_on_fork)
{
	...
	if (ghost_policy(oldpolicy)) {
	    ...
	    {
		/*
		 * XXX temporary warning to investigate an unexpected
		 * MSG_TASK_DEPARTED in SLL.
		 */
		struct task_struct *agent = rq->ghost.agent;
		WARN_ON_ONCE(agent &&
		  strncmp(agent->comm, "agent_muppet", TASK_COMM_LEN) == 0 &&
		  strncmp(current->comm, "agent_muppet", TASK_COMM_LEN) != 0);
	    }
	...
	f = fdget(ghost_schedattr_to_fd(attr));
	e = fd_to_enclave(f);
	if (!e) {
		fdput(f);
		return -EBADF;
...
static void enclave_add_queue(struct ghost_enclave *e, struct ghost_queue *q)
{
	ulong flags;
	...
	if (e->def_q == NULL)
		e->def_q = q;
	list_add_tail(&q->list, &e->queue_list);
	spin_unlock_irqrestore(&e->lock, flags);
}

static void enclave_del_queue(struct ghost_enclave *e, struct ghost_queue *q)
{
	ulong flags;
...
static inline int _produce_for_task(struct task_struct *p, int type,
				    void *payload, int payload_size,
				    bool inc_barrier)
{
	if (inc_barrier)
...
static inline int produce_for_task(struct task_struct *p, int type,
				   void *payload, int payload_size)
{
	return _produce_for_task(p, type, payload, payload_size, true);
}

static inline int _produce_for_agent(struct rq *rq, int type, void *payload,
				     int payload_size, bool inc_barrier)
{
	struct task_struct *agent = rq->ghost.agent;
...
static inline int produce_for_agent(struct rq *rq, int type, void *payload,
				    int payload_size)
{
	return _produce_for_agent(rq, type, payload, payload_size, true);
}
				 ...
				 struct timerfd_ghost __user *utfdl)
{
	...
	if (get_itimerspec64_ghost(&new, utmr))
		return -EFAULT;
	...
	if (otmr && put_itimerspec64_ghost(&old, otmr))
		return -EFAULT;
